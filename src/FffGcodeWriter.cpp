...
void FffGcodeWriter::processRaft(const SliceDataStorage& storage)
{
    ...
    { // raft base layer
        ...
        const AngleDegrees fill_angle = fill_angles.empty() ? AngleDegrees(0.) : fill_angles.at(layer_nr % fill_angles.size());
    ...
    { // raft interface layer
        ...
        const std::vector<AngleDegrees> fill_angles = train.settings.get<std::vector<AngleDegrees>>("raft_angles");
        const AngleDegrees fill_angle = fill_angles.empty() ? AngleDegrees(train.settings.get<size_t>("raft_surface_layers") > 0 ? 45. : 90.) : fill_angles.at(layer_nr % fill_angles.size());
    ...
    { // raft surface layers
        ...
        int offset_from_poly_outline = 0;
        const std::vector<AngleDegrees> fill_angles = train.settings.get<std::vector<AngleDegrees>>("raft_angles");
        const AngleDegrees fill_angle = fill_angles.empty() ? AngleDegrees(90. * raft_surface_layer) : fill_angles.at(layer_nr % fill_angles.size());
        constexpr bool zig_zaggify_infill = true;
